...
| Detect linked CAD geometry | unchecked | If checked, the filter also matches `ImportInstance` elements (linked DWG / DGN / SAT — covers STEP and IFC files brought through AutoCAD into a DWG). `FindReferenceTarget` switches from `Face` to `All` and `FindNearest` then picks the **absolute closest hit of any reference type** — face, mesh, or edge. CAD imports often only expose edge references (especially STEP-via-AutoCAD wireframes); Face-only mode would miss those entirely, and a face-or-mesh-only filter would let the ray pass through an intervening beam to whatever is above. The DWG must be **visible in the `3D-Raybounce` view** for the hit to land; if its import subcategory is hidden in V/G the ray sails right through |
