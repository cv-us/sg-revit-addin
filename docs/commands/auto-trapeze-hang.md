...
**Command class:** `SSG_FP_Suite.Commands.Hangers.AutoTrapezeHangCommand`
...
|---------|-------------------|----------------|
| Speed | Very slow — BimorphNodes geometry analysis + Python sorting | Fast — analytical distance calculation |
| Dependencies | Data-Shapes + BimorphNodes + Clockwork + 18 custom .dyf nodes | Zero external dependencies |
| UI | 15-input Data-Shapes form with 2 potential dialogs | 1 consolidated WinForms dialog |
| Transactions | Multiple | Single transaction |
| Structural detection | BimorphNodes BoundingBox.GetElementsIntersect + surface analysis | Direct bounding-box distance calculation |
| Beam width | Full surface geometry via CenterLinesAndWidthsBySurface.dyf | Estimated from bounding box |
| Grid-based spacing | Supported (intersect grids with pipe plane) | Not yet (planned) |
| Settings persistence | Revit Global Parameters (9 params) | Not yet (planned: settings file) |
| Pipe sorting | Python sort by Z, Y, X | Standard LINQ filtering |
