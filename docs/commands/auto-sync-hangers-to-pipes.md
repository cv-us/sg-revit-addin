...
**Command:** `AutoSyncHangersToPipesCommand`
**Domain:** Hangers
**Ribbon:** SSG FP Suite > Hangers > Sync to Pipes
**Migrated from:** `AutoSync - Hangers To Pipes.dyn`
...
- The Dynamo version used `FamilyInstance.SetRotation` (absolute); the C# version computes a delta rotation from the current angle for correctness
