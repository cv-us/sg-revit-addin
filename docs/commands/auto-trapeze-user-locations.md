...
**Command class:** `SSG_FP_Suite.Commands.Hangers.AutoTrapezeUserLocationsCommand`
...
| Feature | Dynamo Script (V12) | Plugin Command |
|---------|-------------------|----------------|
| Speed | Slow — Data-Shapes UI + Curve.Project + surface analysis | Fast — analytical 2D intersection + distance calc |
| Dependencies | Data-Shapes + BimorphNodes + Clockwork | Zero external dependencies |
| UI | 12-input Data-Shapes form | Single WinForms dialog |
| Selection | Combined select then separate by "Pipe" string match | Combined select then separate by category ID |
| Intersection | Curve.Project with CrossProduct replication | IntersectionHelpers analytical segment intersection |
| Structural detection | BimorphNodes surface analysis + ClosestObject | Direct bounding-box distance calculation |
