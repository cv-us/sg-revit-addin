...
**Command class:** `SSG_FP_Suite.Commands.Hangers.AutoHangDownstreamCommand`
**Migrated from:** `Auto Hang - Threaded Branchlines - On Downstream Ends.dyn` (V18)
...
|---------|-------------------|----------------|
| Speed | Slow — BimorphNodes raybounce per point | Fast — native ReferenceIntersector |
| Dependencies | Data-Shapes + BimorphNodes + Genius Loci | Zero external dependencies |
| UI | 7-input Data-Shapes dialog | 1 consolidated dialog |
| Transactions | Multiple per Dynamo node | Single transaction |
| POL detection | Python ConnectorManager script | Native C# ConnectorManager |
| 3D view setup | Complex node chain (hide all, re-enable) | Single helper method |
| Global Parameters | Settings stored as Revit Global Parameters | Not yet (planned: settings file) |
| Hydratec params | "Type Code (Hydratec)", "Additional Stocklist Info" | Generic "Type Code", "Comments" |
| Disconnected pipe warning | TaskDialog + highlight in selection | Filtered out silently (no crash) |
| Extra midpoint hanger | < 1.5" dia AND > 12' long | Same logic preserved |
