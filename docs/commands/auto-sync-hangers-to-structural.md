...
**Command:** `AutoSyncHangersToStructuralCommand`
**Domain:** Hangers
**Ribbon:** SSG FP Suite > Hangers > Sync to Structural
**Migrated from:** `AutoSync - Hangers To Structural Elements - 3DRayBounce.dyn`
...
### Revit API Method
Uses `ReferenceIntersector` (the API equivalent of Dynamo's RayBounce):
...
- The Dynamo version saved type code defaults to Global Parameters; this C# version uses dialog defaults (enhancement opportunity: persist to project parameters)
- The Dynamo version had a custom "RayBounce On Link Category" node; the C# version uses the built-in `ReferenceIntersector.FindReferencesInRevitLinks` property which achieves the same result natively
