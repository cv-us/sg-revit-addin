...
**Command class:** `SSG_FP_Suite.Commands.Hangers.AutoHangAtCADLinesCommand`
**Migrated from:** `Auto Hang - Pipes Crossing Linked CAD File Lines.dyn` (V12)
...
|---------|--------------|----------------|
| Speed | Slow on large models (geometry projection per pair) | Fast — analytical 2D intersection math with bounding box pre-filter |
| Dependencies | Requires Data-Shapes + Genius Loci packages | Zero external dependencies |
| UI | 4 sequential dialogs | 1 consolidated dialog |
| Transactions | Multiple (per Dynamo node) | Single transaction (all-or-nothing) |
| Settings persistence | Global Parameters | Not yet (planned) |
| Pipe type filter | Yes (by type code) | Not yet (pre-select in Revit instead) |
| Stocklist info parameter | Yes (Hydratec-specific) | Not yet (configurable, planned) |
