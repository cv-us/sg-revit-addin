...
**Command:** `InsertFlexDropLengthsDalmatianCommand`
**Domain:** Annotation
**Ribbon:** SSG FP Suite > Annotation > Flex Drop Dalmatian
**Migrated from:** `Custom AutoInsert - Flexible Drop Lengths-Dalmatian Fire Style.dyn` (V04)
...
| Feature | Standard (`InsertFlexDropLengthsCommand`) | Dalmatian (this command) |
|---------|------------------------------------------|--------------------------|
...
- The Dynamo version stored tag orientation in a Revit Global Parameter (`"Flexible Drop Tag Orientation"`); the C# version uses dialog defaults
- The Dynamo version used Data-Shapes for UI; the C# version uses WinForms
