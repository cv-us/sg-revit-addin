...
**Command:** `CopyLinkLevelsAndGridsCommand`
**Domain:** Setup
**Ribbon:** SSG FP Suite > Setup > Copy Link Levels/Grids
**Migrated from:** `! Setup - Copy Link Levels and Grids.dyn` (V31)
...
## Comparison with Dynamo Version

| Feature | Dynamo (V31) | C# Command |
|---------|-------------|------------|
| Link element copy | BimorphNodes `LinkElement.CopyToActiveDocument` | Native `Level.Create` / `Grid.Create` |
| Grid type change | `Element.SetParameterByName("Type")` | `ChangeTypeId()` (more reliable) |
| Duplicate detection | CrossProduct `List.Equals` + `FilterByBoolMask` | `HashSet<string>.Contains` (faster) |
| UI | Data-Shapes multi-step forms | WinForms dialog + checklist |
| Summary | 4 separate TaskDialogs | Single consolidated summary |

## Notes

- The Dynamo version used BimorphNodes which internally used `ElementTransformUtils.CopyElements`; the C# version creates elements natively which gives more control over the result
- Grid curves are properly transformed from linked model coordinates to host model coordinates
- The elevation formatter matches the Dynamo Python script: whole numbers show without decimals, others strip trailing zeros
