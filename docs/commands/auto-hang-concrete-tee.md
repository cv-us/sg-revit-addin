...
**Command:** `AutoHangConcreteTeeCommand`
**Domain:** Hangers
**Ribbon:** SSG FP Suite > Hangers > Hang Tee Stems
**Migrated from:** `Custom Auto Hang - Concrete Tees (Warped) - Side of Stems - User Locations.dyn` (V21)
...
4. Samples each face at UV parameters (0.5, 0.2) and (0.5, 0.8) to create a center line — matching the Dynamo script's `Surface.PointAtParameter` approach
...
- The Dynamo version used BimorphNodes for linked element extraction; the C# version uses native Revit API `RevitLinkInstance.GetLinkDocument()` and `FilteredElementCollector`
- The Dynamo version stored settings in Revit Global Parameters; the C# version uses dialog defaults
