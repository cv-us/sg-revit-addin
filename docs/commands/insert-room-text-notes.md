...
**Command:** `InsertRoomTextNotesCommand`
**Domain:** Annotation
...
## Improvement Over Dynamo

The Dynamo script would create duplicate text notes on every re-run. This command adds a "Delete existing" option that removes all text notes of the selected type from the active view before placing new ones, enabling clean re-runs without manual cleanup.
