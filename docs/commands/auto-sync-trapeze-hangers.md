...
**Command:** `AutoSyncTrapezeHangersCommand`
**Domain:** Hangers
...
- The "3D-Raybounce" view is shared with `AutoSyncHangersToStructuralCommand`
- The Dynamo original used a series of Python scripts and custom nodes; the C# version consolidates all logic into a single `ProcessHanger()` method
- Minimum clearance default of 7" matches the Dynamo script's default
