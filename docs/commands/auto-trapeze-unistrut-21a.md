...
**Command class:** `SSG_FP_Suite.Commands.Hangers.AutoTrapezeUnistrut21ACommand`
**Migrated from:** `Auto Trapeze Hang - Unistrut 21A - Auto Spaced.dyn` (V28)
...
Example: `"04;21A"` — where "04" is the pipe hanger type and "21A" is the trapeze type. This matches the format used by the original Dynamo scripts.
...
|---------|-------------------|----------------|
| Speed | Slow — 11 UI inputs + BimorphNodes | Fast — native C# |
| Dependencies | Data-Shapes + BimorphNodes + Clockwork | Zero external dependencies |
| UI | Data-Shapes form | Single WinForms dialog |
| User options | Simplified (fewer inputs) | Full options with 21A defaults |
| Grid-based spacing | Supported | Not yet (planned) |
