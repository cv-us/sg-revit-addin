...
**Command class:** `SSG_FP_Suite.Commands.Annotation.InsertGraphicScaleBarsCommand`
...
|---------|--------------|----------------|
| Speed | Slow — processes all sheets | Fast — native C# |
| Sheet selection | All sheets only | All or specific sheets |
| Dependencies | Custom .dyf nodes | Zero |
| UI | No interactive UI | WinForms dialog with sheet picker |
| Global param storage | Used for persistence | Not needed |
| Family path fallback | Hardcoded C:\BIM Support path | Requires family pre-loaded |
