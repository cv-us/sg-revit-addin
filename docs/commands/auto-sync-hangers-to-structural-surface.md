...
**Command:** `AutoSyncHangersToStructuralSurfaceCommand`
**Domain:** Hangers
...
This matches the Dynamo script's "Quick Fix" filter groups that excluded non-hangable structural members.
...
- The Dynamo version used BimorphNodes' `BoundingBox.GetElementsIntersect` for spatial search and `Geometry.Intersect` for line-surface intersection; the C# version uses native bounding box comparison and face tessellation with point-in-polygon testing
- The Dynamo version had separate Tekla vs Revit framing filtering paths; the C# version uses a unified family name exclusion filter
- The Dynamo version used `LinkElementTransformedSurfaces` custom node; the C# version applies `link.GetTotalTransform()` directly to extracted geometry
