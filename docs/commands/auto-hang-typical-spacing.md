...
**Command class:** `SSG_FP_Suite.Commands.Hangers.AutoHangTypicalSpacingCommand`
...
| Feature | Dynamo Script (V18) | Plugin Command |
|---------|-------------------|----------------|
| Speed | Slow — LoopWhile nodes, BimorphNodes raybounce | Fast — direct iteration, native ReferenceIntersector |
| Dependencies | Data-Shapes + BimorphNodes + 12 custom .dyf nodes | Zero external dependencies |
| UI | 10-input Data-Shapes form with radio buttons | 1 consolidated WinForms dialog |
| Transactions | Multiple per Dynamo node | Single transaction |
| Coplanar grouping | Complex LoopWhile sorting pipes by plane | Not needed — processes pipes individually |
| Pipe direction normalization | 12-node vector correction chain | Built into spacing calculation |
| Global Parameters | 9 Revit Global Parameters for persistence | Not yet (planned: settings file) |
| Revit version handling | `Version<2020? "Start Offset" : "Start Middle Elevation"` | Direct Z coordinate from placement point |
| Structural detection | BimorphNodes bounding box intersection + surface filtering | Native ReferenceIntersector (simpler, faster) |
