...
**Command:** `AutoShortenFlexPipesCommand`
**Domain:** PipeRouting
**Ribbon:** SSG FP Suite > Pipe Routing > Shorten Flex Pipes
**Migrated from:** `AutoShorten - Flex Pipes.dyn`
...
Connected elements must be real physical elements (fittings, sprinkler heads, pipe segments), not PipingSystem objects. The Dynamo script converts connected elements to strings and checks for "PipingSystem" — this command checks the element type directly.
...
- The original Dynamo script used MEPover's `FlexCurve.ByElements` node; the C# version uses `FlexPipe.Create` with explicit XYZ points plus `Connector.ConnectTo` for reconnection
